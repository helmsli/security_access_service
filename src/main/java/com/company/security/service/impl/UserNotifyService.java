...
public class UserNotifyService {
	...
	public static final String key_userLoginNoty = "userLoginNotify";
	public static final String key_userId = "userId";
	public static final String key_code = "inviteNo";

	private String category_modify_user = "category_modifyUser";
	
	private String category_login_user = "category_loginUser";
	@Autowired
	 ...
	 */
	@Async
	public void notifyLoginUserAsync(long userId, String loginUserInfo) {
		for (int i = 0; i < 3; i++) {
			ProcessResult ret = notifyRegisterUser(userId, loginUserInfo);
			if (ret.getRetCode() == 0) {
				return;
			}
			try {
	...
	protected ProcessResult notifyLoginUser(long lUserId, String loginInfo) {

		String userId = Long.toString(lUserId);
		String orderId = this.orderClientService.getOrderId(category_login_user, userId);
		if (orderId == null) {
			logger.error("get orderId error:" + userId);
			return ControllerUtils.getErrorResponse(-1, "get orderId error");
		}
		OrderMainContext orderMainContext = new OrderMainContext();
		orderMainContext.setCatetory(category_login_user);
		orderMainContext.setOwnerKey(userId);
		orderMainContext.setOrderId(orderId);
		Map<String, String> contextDatas = new HashMap<String, String>();

		Map<String, String> notifyMap = new HashMap<String, String>();
		notifyMap.put("loginInfo", loginInfo);
		notifyMap.put(this.key_userId, userId);
		notifyMap.put("loginTime",String.valueOf(System.currentTimeMillis()));
		contextDatas.put(this.key_userLoginNoty, JsonUtil.toJson(notifyMap));
		orderMainContext.setContextDatas(contextDatas);
		ProcessResult ret = this.orderClientService.createOrder(orderMainContext);
		if (ret.getRetCode() != 0) {
			return ret;
		}
		ret = this.orderClientService.startOrder(category_login_user, orderId);
		return ret;
	}
}
